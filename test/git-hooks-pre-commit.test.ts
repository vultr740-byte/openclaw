...
describe("git-hooks/pre-commit (integration)", () => {
  ...
  it("restages explicitly tracked files even when they match .gitignore", () => {
    const dir = makeTempRepoRoot(tempDirs, "openclaw-pre-commit-ignored-");
    run(dir, "git", ["init", "-q", "--initial-branch=main"]);

    const fakeBinDir = installPreCommitFixture(dir);
    writeExecutable(fakeBinDir, "pnpm", "#!/usr/bin/env bash\nexit 0\n");

    writeFileSync(path.join(dir, ".gitignore"), ".agents/\n", "utf8");
    mkdirSync(path.join(dir, ".agents", "skills"), { recursive: true });
    writeFileSync(path.join(dir, ".agents", "skills", "tracked.txt"), "hello\n", "utf8");
    run(dir, "git", ["add", "-f", "--", ".gitignore", ".agents/skills/tracked.txt"]);
    ...
    const staged = run(dir, "git", ["diff", "--cached", "--name-only"]).split("\n").filter(Boolean);
    expect(staged).toEqual([".agents/skills/tracked.txt", ".gitignore"]);
  });
});
