...
If output shows `UPGRADE_AVAILABLE <old> <new>`: read `skills/gstack-upgrade/SKILL.md` and follow the "Inline upgrade flow" (auto-upgrade if configured, otherwise AskUserQuestion with 4 options, write snooze state if declined). If `JUST_UPGRADED <from> <to>`: tell user "Running gstack v{to} (just updated!)" and continue.
...
If A: run `skills/gstack/bin/gstack-config set telemetry anonymous`
If B: run `skills/gstack/bin/gstack-config set telemetry off`
...
1. Read `skills/gstack/review/checklist.md`. If the file cannot be read, **STOP** and report the error.
...
2. **Read `skills/gstack/review/design-checklist.md`.** If the file cannot be read, skip design review with a note: "Design checklist not found — skipping design review."
...
Read `skills/gstack/review/greptile-triage.md` and follow the fetch, filter, classify, and **escalation detection** steps.
...
```bash
skills/gstack/bin/gstack-review-log '{"skill":"codex-review","timestamp":"TIMESTAMP","status":"STATUS","gate":"GATE"}'
```
...
Read `skills/gstack/review/TODOS-format.md` for the canonical format reference.
...
`skills/gstack-document-release/SKILL.md` skill file and
execute its full workflow:

1. Read the `/gstack-document-release` skill: `cat skills/gstack-document-release/SKILL.md`
