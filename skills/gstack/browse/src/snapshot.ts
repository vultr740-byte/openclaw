...
export const SNAPSHOT_FLAGS: Array<{
  ...
  },
  {
    short: "-o",
    long: "--output",
    description: "Output path for annotated screenshot (default: /tmp/browse-annotated.png)",
    takesValue: true,
    valueHint: "<path>",
...
export async function handleSnapshot(args: string[], bm: BrowserManager): Promise<string> {
  ...
  if (opts.annotate) {
    ...
    if (
      !safeDirs.some((dir: string) => resolvedPath === dir || resolvedPath.startsWith(dir + "/"))
    ) {
