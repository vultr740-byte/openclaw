...
Note: in this repo-local OpenClaw vendoring, `/gstack-upgrade` refreshes the
support sidecar at `skills/gstack`. It does not regenerate the sibling
`gstack-*` skill markdown files automatically.
...
```bash
if [ -d "skills/gstack/.git" ]; then
  INSTALL_TYPE="local-git"
  INSTALL_DIR="skills/gstack"
elif [ -d "skills/gstack" ]; then
...
if [ "$INSTALL_DIR" = "skills/gstack" ]; then
  GSTACK_LOCAL_ONLY=1 ./setup
else
  ./setup
fi
```
...
if [ "$INSTALL_DIR" = "skills/gstack" ]; then
  (cd "$INSTALL_DIR" && GSTACK_LOCAL_ONLY=1 ./setup)
else
...
LOCAL_GSTACK=""
if [ -n "$_ROOT" ] && [ -d "$_ROOT/skills/gstack" ]; then
  _RESOLVED_LOCAL=$(cd "$_ROOT/skills/gstack" && pwd -P)
  _RESOLVED_PRIMARY=$(cd "$INSTALL_DIR" && pwd -P)
  if [ "$_RESOLVED_LOCAL" != "$_RESOLVED_PRIMARY" ]; then
    LOCAL_GSTACK="$_ROOT/skills/gstack"
  fi
fi
...
cd "$LOCAL_GSTACK" && GSTACK_LOCAL_ONLY=1 ./setup
