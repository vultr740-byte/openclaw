...
3. `/codex` with no arguments — **Auto-detect:**
   ...
   - If a diff exists, use AskUserQuestion:
     ...
     ```
   - If no diff, check the local Codex plan directory scoped to the current project:
     ```bash
     [ -d "$HOME/.codex/plans" ] && ls -t "$HOME/.codex/plans"/*.md 2>/dev/null | xargs grep -l "$(basename "$(pwd)")" 2>/dev/null | head -1
     ```
     If no project-scoped match, fall back to the newest file from the same directory:
     ```bash
     [ -d "$HOME/.codex/plans" ] && ls -t "$HOME/.codex/plans"/*.md 2>/dev/null | head -1
     ```
