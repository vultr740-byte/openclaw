...
function appendClawHubHint(output: string, json?: boolean): string {
  ...
  return `${output}\n\nTip: use the native \`skillhub\` tool or \`openclaw skills search/install/update\` for ClawHub-backed skills; \`npx clawhub\` remains available as a legacy fallback.`;
}
