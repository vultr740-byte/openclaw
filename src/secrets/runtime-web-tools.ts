...
export async function resolveRuntimeWebTools(params: {
  ...
  const legacyXSearchSource = isRecord((sourceWeb as { x_search?: unknown } | undefined)?.x_search)
    ? ((sourceWeb as { x_search?: unknown }).x_search as LegacyXSearchConfig)
    : undefined;
  const legacyXSearchResolved = isRecord(
    (resolvedWeb as { x_search?: unknown } | undefined)?.x_search,
  )
    ? ((resolvedWeb as { x_search?: unknown }).x_search as LegacyXSearchConfig)
    : undefined;
