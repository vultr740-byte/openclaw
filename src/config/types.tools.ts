...
export type ToolsConfig = {
  ...
  web?: {
    search?: {
      ...
      enabled?: boolean;
      /** Search provider ("openai", "brave", "perplexity", "grok", "gemini", or "kimi"). */
      provider?: "openai" | "brave" | "perplexity" | "grok" | "gemini" | "kimi";
      ...
      openai?: {
        /** API key for OpenAI (defaults to OPENAI_API_KEY or models.providers.openai.apiKey). */
        apiKey?: string;
        /** Base URL for OpenAI requests (default: https://api.openai.com/v1). */
        baseUrl?: string;
        /** Model to use (defaults to the current agent model when possible). */
        model?: string;
      };
