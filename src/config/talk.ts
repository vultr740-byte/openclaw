...
export function normalizeTalkSection(value: TalkConfig | undefined): TalkConfig | undefined {
  ...
  const legacyCompat = buildLegacyTalkProviderCompat(source);
  if (providers) {
  ...
  if (!normalized.provider && !normalized.providers && legacyCompat) {
    normalized.provider = DEFAULT_TALK_PROVIDER;
    normalized.providers = { [DEFAULT_TALK_PROVIDER]: legacyCompat };
  }
...
export function buildTalkConfigResponse(value: unknown): TalkConfigResponse | undefined {
  ...
  const resolved =
    ...
    (legacyCompat ? { provider: DEFAULT_TALK_PROVIDER, config: legacyCompat } : undefined);
...
function readTalkApiKeyFromProfile(deps: TalkApiKeyDeps = {}): string | null {
  const fsImpl = deps.fs;
  const osImpl = deps.os;
  const pathImpl = deps.path;
  if (!fsImpl || !osImpl || !pathImpl) {
    return null;
  }
