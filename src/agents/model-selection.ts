...
export function buildConfiguredAllowlistKeys(params: {
  ...
  for (const raw of rawAllowlist) {
    const key = resolveAllowlistModelKey(resolveConfiguredModelKey(raw), params.defaultProvider);
    if (key) {
      keys.add(key);
    }
  }
...
export function buildModelAliasIndex(params: {
  ...
  for (const [keyRaw, entryRaw] of Object.entries(rawModels)) {
    const parsed = parseModelRef(resolveConfiguredModelKey(keyRaw), params.defaultProvider, {
...
function buildModelCatalogMetadata(params: {
  ...
  for (const [rawKey, entryRaw] of Object.entries(configuredModels)) {
    const key = resolveAllowlistModelKey(resolveConfiguredModelKey(rawKey), params.defaultProvider);
    if (!key) {
      continue;
    }
...
export function buildAllowedModelSet(params: {
  ...
  for (const raw of rawAllowlist) {
    const parsed = parseModelRef(resolveConfiguredModelKey(raw), params.defaultProvider);
    if (!parsed) {
      continue;
    }
    const key = modelKey(parsed.provider, parsed.model);
