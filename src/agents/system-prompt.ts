...
function buildDocsSection(params: { docsPath?: string; isMinimal: boolean; readToolName: string }) {
  ...
  return [
    ...
    "Find new skills: use the built-in skillhub tool first; ClawHub remains a legacy fallback at https://clawhub.com",
...
export function buildAgentSystemPrompt(params: {
  ...
  const coreToolSummaries: Record<string, string> = {
    ...
    web_search: "Search the web (OpenAI, Brave, Perplexity, or Grok)",
    ...
    followup:
      "Schedule a follow-up polling job when you promise to reply later (fixed 30s interval, max 10 minutes; respond HEARTBEAT_OK while waiting)",
