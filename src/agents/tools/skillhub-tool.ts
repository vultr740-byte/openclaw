...
export type SkillhubSearchMatch = NormalizedSkillHubSearchResult;
...
async function loadSearchMatches(params: {
  config: ResolvedSkillHubConfig;
  query: string;
  limit: number;
...
export async function searchSkillhubMatches(params: {
  config?: OpenClawConfig;
  query: string;
  limit?: number;
}): Promise<SkillhubSearchMatch[]> {
  const config = resolveSkillHubConfig(params.config);
  assertSkillHubEnabled(config);
  return await loadSearchMatches({
    config,
    query: params.query,
...
async function runSearch(params: { config: ResolvedSkillHubConfig; query: string; limit: number }) {
  const matches = await loadSearchMatches(params);
  return jsonResult({
    provider: "skillhub",
    query: params.query,
