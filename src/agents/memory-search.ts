...
function mergeConfig(
  ...
  const multimodal = normalizeMemoryMultimodalSettings({
    ...
    modalities: (overrides?.multimodal?.modalities ?? defaults?.multimodal?.modalities)?.filter(
      (entry): entry is "image" | "audio" => entry === "image" || entry === "audio",
    ),
  ...
  const fts = {
    tokenizer: rawTokenizer === "trigram" ? "trigram" : "unicode61",
  } as const;
  const store = {
