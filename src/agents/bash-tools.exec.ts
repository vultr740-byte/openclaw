...
const USER_OVERRIDABLE_RUNTIME_ENV_KEYS = new Set([
  "HOME",
  "XDG_CONFIG_HOME",
  "XDG_CACHE_HOME",
  "XDG_STATE_HOME",
...
function partitionRequestedEnvOverrides(params: {
  requestedEnvOverrides?: Record<string, string>;
  runtimeEnvDefaults?: Record<string, string>;
}): {
  hostValidatedOverrides?: Record<string, string>;
  runtimeEnvOverrides?: Record<string, string>;
} {
  const requestedEnvOverrides = params.requestedEnvOverrides;
  const runtimeEnvDefaults = params.runtimeEnvDefaults;
  if (!requestedEnvOverrides) {
    return {};
  }

  const runtimeEnvKeyMap = new Map<string, string>();
  for (const key of Object.keys(runtimeEnvDefaults ?? {})) {
    const normalized = key.trim().toUpperCase();
    if (!normalized || !USER_OVERRIDABLE_RUNTIME_ENV_KEYS.has(normalized)) {
      continue;
    }
    runtimeEnvKeyMap.set(normalized, key);
  }

  if (runtimeEnvKeyMap.size === 0) {
    return {
      hostValidatedOverrides: requestedEnvOverrides,
    };
  }

  const hostValidatedOverrides: Record<string, string> = {};
  const runtimeEnvOverrides: Record<string, string> = {};

  for (const [rawKey, value] of Object.entries(requestedEnvOverrides)) {
    if (typeof value !== "string") {
      continue;
    }
    const normalized = rawKey.trim().toUpperCase();
    const canonicalRuntimeKey = runtimeEnvKeyMap.get(normalized);
    if (canonicalRuntimeKey) {
      runtimeEnvOverrides[canonicalRuntimeKey] = value;
      continue;
    }
    hostValidatedOverrides[rawKey] = value;
  }

  return {
    hostValidatedOverrides:
      Object.keys(hostValidatedOverrides).length > 0 ? hostValidatedOverrides : undefined,
    runtimeEnvOverrides:
      Object.keys(runtimeEnvOverrides).length > 0 ? runtimeEnvOverrides : undefined,
  };
}
...
export function createExecTool(
  ...
  return {
    ...
    execute: async (_toolCallId, args, signal, onUpdate) => {
      ...
      const requestedEnvOverrides = params.env ? { ...params.env } : undefined;
      const { hostValidatedOverrides: hostValidatedRequestedEnvOverrides, runtimeEnvOverrides } =
        partitionRequestedEnvOverrides({
          requestedEnvOverrides,
          runtimeEnvDefaults,
        });
      const resolvedRuntimeEnv = mergeEnvEntries(runtimeEnvDefaults, runtimeEnvOverrides);
      const trustedInternalEnv = mergeEnvEntries(
        resolvedRuntimeEnv,
        installEnvRedirect,
        installRewriteEnv,
      );
      ...
      const hostEnvResult =
        host === "sandbox"
          ...
          : sanitizeHostExecEnvWithDiagnostics({
              ...
              overrides: hostValidatedRequestedEnvOverrides,
              blockPathOverrides: true,
            });
      if (
        hostEnvResult &&
        hostValidatedRequestedEnvOverrides &&
      ...
      const env =
        sandbox && host === "sandbox"
          ? buildSandboxEnv({
              ...
              paramsEnv: mergeEnvEntries(hostValidatedRequestedEnvOverrides, trustedInternalEnv),
              sandboxEnv: sandbox.env,
              containerWorkdir: containerWorkdir ?? sandbox.containerWorkdir,
            })
          : (() => {
              const resolved = { ...(hostEnvResult?.env ?? inheritedBaseEnv) };
              if (trustedInternalEnv) {
                Object.assign(resolved, trustedInternalEnv);
              }
              return resolved;
            })();
      ...
      if (host === "node") {
        return executeNodeHostCommand({
          ...
          workdir,
          env,
          requestedEnv: requestedEnvOverrides,
