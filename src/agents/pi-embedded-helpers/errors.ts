...
const HTTP_STATUS_PREFIX_RE = /^(?:http\s*)?(\d{3})(?:\s*:\s*|\s+)(.+)$/i;
...
const RAW_402_MARKER_RE =
  /["']?(?:status|code)["']?\s*[:=]\s*402\b|\bhttp\s*402\b|\berror(?:\s+code)?\s*[:=]?\s*402\b|\b(?:got|returned|received)\s+(?:a\s+)?402\b|^\s*402\s+(?:payment required|status code)\b|^\s*402\s+.*used up your points\b/i;
