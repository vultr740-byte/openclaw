...
describe("exec PATH login shell merge", () => {
  ...
  it("keeps HOME and XDG_CONFIG_HOME blocked when runtime install env is absent", async () => {
    if (isWin) {
      return;
    }
    const tool = createExecTool({ host: "gateway", security: "full", ask: "off" });
    ...
    await expect(
      tool.execute("call-blocked-home-env", {
        command: "echo ok",
        env: {
          HOME: "/tmp/evil-home",
    ...
    ).rejects.toThrow(/Security Violation: blocked override keys: HOME, XDG_CONFIG_HOME\./);
  });
