...
function applyOutcomeToStoredJob(state: CronServiceState, result: TimedCronRunOutcome): void {
  ...
  if (shouldDelete || followupExpired || stopFollowup) {
...
export async function onTimer(state: CronServiceState) {
  ...
  try {
    const dueJobs = await locked(state, async () => {
      ...
      const expired = state.store?.jobs.filter((j) => isFollowupExpired(j, dueCheckNow)) ?? [];
      ...
      for (const job of due) {
        job.state.runningAtMs = dueCheckNow;
...
async function runStartupCatchupCandidate(
  ...
  try {
    ...
    return {
      ...
      heartbeatOnly: result.heartbeatOnly,
      delivered: result.delivered,
