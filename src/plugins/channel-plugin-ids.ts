...
export function resolveGatewayStartupPluginIds(params: {
  ...
  return loadPluginManifestRegistry({
    ...
    .plugins.filter((plugin) => {
      ...
      // Channel plugins can expose setup/login gateway routes before credentials
      // exist, so keep explicitly enabled channel plugins in the startup scope.
      if (plugin.channels.length > 0) {
        return activationState.enabled && activationState.source === "explicit";
      }
      if (
