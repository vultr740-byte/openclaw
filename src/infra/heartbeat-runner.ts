...
function resolveHeartbeatReasonFlags(reason?: string): HeartbeatReasonFlags {
  ...
  const isFollowupEventReason = Boolean(reason?.trim().startsWith("followup:"));
  return {
...
function resolveHeartbeatRunPrompt(params: {
  ...
  const pendingEvents = params.preflight.shouldInspectPendingEvents
    ...
    : [];
  const cronEvents = params.preflight.shouldInspectPendingEvents
    ? pendingEventEntries
        .filter(
          (event) =>
...
export async function runHeartbeatOnce(opts: {
  ...
  const isFollowupEvent = preflight.isFollowupEventReason;
  const ctx = {
