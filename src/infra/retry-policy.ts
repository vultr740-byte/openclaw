...
function isRateLimitErrorLike(err: unknown): err is RateLimitErrorLike {
  return (
    typeof err === "object" &&
    err !== null &&
...
export function createDiscordRetryRunner(params: {
  ...
  return createRateLimitRetryRunner({
    ...
    logLabel: "discord",
    shouldRetry: (err) => isRateLimitErrorLike(err),
    retryAfterMs: (err) =>
      isRateLimitErrorLike(err) ? Math.max(0, err.retryAfter ?? 0) * 1000 : undefined,
  });
}

export const TELEGRAM_RETRY_DEFAULTS = CHANNEL_API_RETRY_DEFAULTS;
...
export function createTelegramRetryRunner(params: {
  ...
  return createChannelApiRetryRunner(params);
}
