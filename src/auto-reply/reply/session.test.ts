...
describe("initSessionState user activity tracking", () => {
  it("records lastUserMessageAt for real inbound user messages", async () => {
    vi.useFakeTimers();
    try {
      vi.setSystemTime(new Date("2026-01-18T03:00:00Z"));
      const storePath = await createStorePath("openclaw-session-user-activity-");
      const cfg = {
        session: { store: storePath },
      } as OpenClawConfig;
      ...
      expect(result.sessionEntry.lastUserMessageAt).toBe(Date.parse("2026-01-18T03:00:00.000Z"));
      const raw = JSON.parse(await fs.readFile(storePath, "utf-8")) as Record<string, SessionEntry>;
      expect(raw[result.sessionKey]?.lastUserMessageAt).toBe(
        Date.parse("2026-01-18T03:00:00.000Z"),
      );
    } finally {
      vi.useRealTimers();
    }
  ...
  it("preserves lastUserMessageAt during heartbeat/internal session runs", async () => {
    vi.useFakeTimers();
    try {
      vi.setSystemTime(new Date("2026-01-18T04:00:00Z"));
      const storePath = await createStorePath("openclaw-session-user-activity-preserve-");
      const sessionKey = "agent:main:telegram:123";
      await writeSessionStoreFast(storePath, {
        [sessionKey]: {
          sessionId: "session-1",
          updatedAt: Date.parse("2026-01-18T03:30:00.000Z"),
          lastUserMessageAt: Date.parse("2026-01-18T03:00:00.000Z"),
        },
      });
      const cfg = {
        session: { store: storePath },
      ...
      expect(result.sessionEntry.lastUserMessageAt).toBe(Date.parse("2026-01-18T03:00:00.000Z"));
      const raw = JSON.parse(await fs.readFile(storePath, "utf-8")) as Record<string, SessionEntry>;
      expect(raw[sessionKey]?.lastUserMessageAt).toBe(Date.parse("2026-01-18T03:00:00.000Z"));
    } finally {
      vi.useRealTimers();
    }
  });
